...
class RewardRedistributionModel(nn.Module):
    ...
    def get_context_rewards_and_past_baseline(
        self,
        states: torch.Tensor,
        actions: torch.Tensor,
        history_states: torch.Tensor,
        ...
        return context_rewards, past_baseline
...
class HistoryGRURewardRedistributionModel(nn.Module):
    ...
    def get_context_rewards_and_past_baseline(
        ...
        seq_len = states.shape[1]
        offsets = torch.arange(seq_len, device=states.device).unsqueeze(0)
        context_starts = history_context_starts.long()
        gather_idx = context_starts.unsqueeze(1) + offsets
        gather_idx = gather_idx.clamp(max=history_rewards.shape[1] - 1)
        context_rewards = torch.gather(history_rewards, dim=1, index=gather_idx)

        positions = torch.arange(history_rewards.shape[1], device=states.device).unsqueeze(0)
        visible_past_mask = positions < context_starts.clamp_min(0).unsqueeze(1)
        visible_past_baseline = (
            history_rewards * history_mask * visible_past_mask
        ).sum(dim=1)

        missing_prefix_len = (
            start_idxs - history_context_starts.to(start_idxs.dtype)
        ).clamp_min(0.0)
        missing_prefix_baseline = (
            traj_returns * missing_prefix_len / traj_lens.clamp_min(1.0)
        )
        return context_rewards, visible_past_baseline + missing_prefix_baseline
...
def dtrd_returns_and_regularizer(
    ...
    step_rewards, past_baseline = reward_model.get_context_rewards_and_past_baseline(
        states,
        actions,
        history_states,
        history_actions,
    ...
    if detach_reward:
        ...
        past_baseline = past_baseline.detach()
